 ...
 */
class NewsApiMessengerService : Service() {
    companion object {
    ...
    inner class IncomingHandler : Handler() {
        ...
        private fun sendMetaReply(messenger: Messenger) {
            try {
                ...
                val bitmap = BitmapFactory.decodeResource(applicationContext.resources, R.drawable.newsapi_plugin_logo)
        ...
        private fun sendNewsReply(messenger: Messenger, category: String) {
            ...
            apiHandler.fetchTopStories(category)
                    .subscribe({
                                   ...
                                   Log.d("+_", "Got arraylist of size: ${it.articles?.size}")
                                   it.articles?.let {
